...
function generateStudentId() {
  return 'S' + new Date().getFullYear() + Math.floor(1000 + Math.random() * 9000)
}
...
async function onSubmit() {
  ...
  try {
    ...
    snackbar.trigger('Registration submitted! Please select your school.', 'info')
    setTimeout(() => {
  ...
  <div class="min-h-screen flex items-center justify-center relative overflow-hidden">
    ...
    >
      <div class="mb-4 flex flex-col items-center">
        ...
        <p class="text-gray-600 text-sm mt-1 text-center">Create your student account</p>
      </div>
      <form @submit.prevent="onSubmit" class="w-full mt-4 space-y-5">
        ...
        <div>
          <label class="block mb-1 font-semibold text-emerald-800">Student ID</label>
          <input
            :value="studentId"
            type="text"
